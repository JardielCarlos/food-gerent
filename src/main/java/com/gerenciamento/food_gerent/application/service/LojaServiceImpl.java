...
public class LojaServiceImpl implements LojaUseCases {
  ...
  private final LojaRepositoryImpl lojaRepositoryImpl;
  ...
  public List<LojaResponseDTO> getAllLojas() {
    List<Loja> lojas = this.lojaRepositoryImpl.findAll();
    return lojaMapper.toResponseDTOList(lojas);
  }

  @Override
  public LojaResponseDTO getLojaById(UUID id) {
    Loja loja = this.lojaRepositoryImpl.findById(id).orElseThrow(() -> new EntityNotFoundException("Loja não encontrada com o id: " + id));
  ...
  public LojaResponseDTO createLoja(LojaRequestDTO data) {
    Empresa empresa = 
      this.empresaRepositoryImpl.findById(UUID.fromString(data.idEmpresa()))
  ...
  public LojaResponseDTO updateLoja(UUID idLoja, UUID idEmpresa, LojaPatchDTO data) {
    ...
    this.lojaRepositoryImpl.save(lojaAtualizada);
  ...
  public void deleteLoja(UUID idLoja, UUID idEmpresa) {
    ...
    this.empresaRepositoryImpl.save(empresa);
  }
